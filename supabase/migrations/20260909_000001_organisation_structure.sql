...
begin
  ...
  if p_publish then
    ...
    if exists (select 1 from jsonb_array_elements(positions) n where n->>'employeeId' is not null and
      (trim(n->>'department') = '' or (n->>'parentId' is not null and not exists (
        select 1 from jsonb_array_elements(positions) parent
        where parent->>'id' = n->>'parentId' and parent->>'employeeId' is not null
      )))) then
      raise exception 'Every assigned position needs a department and an occupied reporting position, or must be a top-level position.' using errcode = '22023';
    end if;
    update public.employees e set
      role = n->>'title', department = nullif(n->>'department',''), team = nullif(n->>'team',''),
      line_manager_id = (select (parent->>'employeeId')::uuid from jsonb_array_elements(positions) parent where parent->>'id' = n->>'parentId'),
