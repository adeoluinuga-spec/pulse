      ...
      <div className="bg-ink rounded-[20px] p-5">
        ...
        <div className="flex items-center gap-3 bg-white/5 rounded-xl p-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white/40 text-[11px] mt-0.5">{peer.role}</p>
          </div>
          <Users size={14} className="text-white/20 flex-shrink-0" />
        </div>

        <p className="text-white/35 text-xs leading-relaxed mb-5">{reason}</p>

        {/* Actions */}
        <div className="flex gap-2">
          <button className="flex-1 bg-pulse text-white text-sm font-semibold py-2.5 rounded-xl hover:bg-pulse/90 transition-colors">
            Request a session
          </button>
          <button
            onClick={handleDismiss}
            className="px-4 text-white/50 text-sm font-medium py-2.5 rounded-xl border border-white/10 hover:border-white/20 hover:text-white/70 transition-colors"
          >
            Dismiss
          </button>
        </div>
