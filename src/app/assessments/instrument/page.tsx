...
export default function AssessmentInstrumentPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 p-6">
      <div className="rounded-lg border border-paper-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-500">360 instrument</p>
            <h1 className="mt-2 text-2xl font-semibold text-ink-900">Build the assessment instrument</h1>
          </div>
          <div className="rounded-lg border border-paper-200 bg-paper-50 px-3 py-2 text-sm font-semibold text-ink-700">
            {validation.activeScaleCount} active scale items
          </div>
        </div>

        {notice ? <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">{notice}</div> : null}
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1.6fr_0.9fr]">
          <section className="space-y-6">
              ...
              <div className="grid gap-3 md:grid-cols-[1.2fr_1fr_1fr]">
                ...
                <div className="flex items-end">
                  <button
                    type="button"
                    onClick={saveItem}
                    className="w-full rounded-md bg-cobalt px-4 py-2.5 text-sm font-semibold text-white"
                  >
            ...
            <div className="space-y-4">
              {groupedItems.map((competency) => (
                <div key={competency.id} className="rounded-lg border border-paper-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h2 className="text-lg font-semibold text-ink-900">{competency.name}</h2>
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-ink-500">
                        {competency.items.length} active item{competency.items.length === 1 ? "" : "s"}
                      </p>
                    </div>
                    <span className="rounded-full bg-paper-100 px-2 py-1 text-xs font-semibold text-ink-700">
                  ...
                  <div className="mt-3 space-y-2">
                    {competency.items.length === 0 ? (
                      <p className="text-sm text-ink-500">No items yet for this competency.</p>
                    ) : (
                      competency.items.map((item) => (
                        <div key={item.id} className="rounded-lg border border-paper-200 bg-paper-50 p-3">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1">
                              <p className="text-sm font-semibold text-ink-900">{item.body}</p>
                              <p className="mt-1 text-[11px] uppercase tracking-[0.12em] text-ink-500">{item.itemType}</p>
                            </div>
                            <div className="flex gap-2">
                              <button type="button" onClick={() => moveItem(item.id!, "up")} className="rounded-lg border border-paper-200 bg-white px-2 py-1 text-xs font-semibold">↑</button>
                              <button type="button" onClick={() => moveItem(item.id!, "down")} className="rounded-lg border border-paper-200 bg-white px-2 py-1 text-xs font-semibold">↓</button>
                              <button
                                type="button"
                                onClick={() => {
                                ...
                                className="rounded-lg border border-paper-200 bg-white px-2 py-1 text-xs font-semibold"
                              >
                                Edit
                              </button>
                              <button type="button" onClick={() => void deactivateItem(item.id!)} className="rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700">
                                Deactivate
                              </button>
                            </div>
                          </div>
            ...
            <div className="rounded-lg border border-paper-200 bg-white p-4">
              <h2 className="text-lg font-semibold text-ink-900">Standalone open text</h2>
              <div className="mt-3 space-y-2">
                {standaloneTextItems.length === 0 ? (
                  <p className="text-sm text-ink-500">No open-text items yet.</p>
                ) : (
                  standaloneTextItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg border border-paper-200 bg-paper-50 p-3">
                      <p className="text-sm text-ink-700">{item.body}</p>
                      <button type="button" onClick={() => void deactivateItem(item.id!)} className="rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700">Deactivate</button>
                    </div>
                  ))
                )}
              </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-lg border border-paper-200 bg-paper-50 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-500">Readiness</p>
              <div className="mt-3 space-y-2">
                {validation.valid ? (
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">Instrument is ready to open.</div>
                ) : (
                  validation.errors.map((error) => (
                    <div key={error} className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">{error}</div>
                  ))
                )}
              </div>
            </div>
            ...
            <div className="rounded-lg border border-paper-200 bg-white p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-500">Preview</p>
              <div className="mt-3 space-y-4 rounded-lg border border-paper-200 bg-paper-50 p-4">
                {groupedItems.map((competency) => (
                  <div key={competency.id}>
                    <p className="text-sm font-semibold text-ink-900">{competency.name}</p>
                    <div className="mt-2 space-y-2">
                      {competency.items.map((item) => (
                        <div key={item.id} className="rounded-lg border border-paper-200 bg-white p-3">
                          <p className="text-sm text-ink-700">{item.body}</p>
                          <div className="mt-2 flex items-center gap-2 text-xs text-ink-500">
                            ...
                            <span>4</span>
                            <span>5</span>
                            <span className="ml-auto rounded-full border border-paper-200 px-2 py-1">Unable to Observe</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {standaloneTextItems.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-ink-900">Open text</p>
                    <div className="mt-2 rounded-lg border border-paper-200 bg-white p-3 text-sm text-ink-700">
                      {standaloneTextItems.map((item) => (
                        <div key={item.id} className="mb-3 last:mb-0">
                          <p>{item.body}</p>
                          <textarea rows={3} className="mt-2 w-full rounded-md border border-paper-200 bg-paper-50 p-2 text-sm" placeholder="Write your response here" />
                        </div>
                      ))}
                    </div>
                  </div>
