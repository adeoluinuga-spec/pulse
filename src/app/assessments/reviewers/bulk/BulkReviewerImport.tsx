    ...
    <main className="mx-auto w-full max-w-6xl">
      <section className="rounded-lg border border-border bg-card p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-muted">Reviewer setup</p>
            <h1 className="mt-1 text-2xl font-semibold text-ink">Bulk import rater assignments</h1>
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border bg-white px-4 text-sm font-semibold text-ink disabled:opacity-40"
          >
            <Download size={17} />
            Download report
          </button>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1.5fr_1fr_1fr]">
          ...
          <label>
            <span className="text-xs font-semibold text-muted">Rater cap warning</span>
            <input
              type="number"
              min={1}
              value={cap}
          ...
          <label className="flex min-h-11 cursor-pointer items-center justify-center gap-2 self-end rounded-lg border border-border bg-white px-4 text-sm font-semibold text-ink">
            <Upload size={17} />
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-pulse px-4 text-sm font-semibold text-white disabled:opacity-40"
          >
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-ink px-4 text-sm font-semibold text-white disabled:opacity-40"
          >
            Create assignments
          </button>
        </div>
      ...
      {result?.loadWarnings && result.loadWarnings.length > 0 && (
        <section className="mt-4 rounded-lg border border-amber/30 bg-amber-soft p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-amber">Rater load warnings</h2>
              <div className="mt-2 space-y-1 text-sm font-semibold leading-6 text-amber">
      ...
      {result?.rows && (
        <section className="mt-4 overflow-hidden rounded-lg border border-border bg-card shadow-sm">
          <div className="border-b border-border p-4">
            <h2 className="text-lg font-semibold text-ink">Row validation</h2>
            <p className="mt-1 text-sm text-muted">{result.rows.length} row{result.rows.length === 1 ? "" : "s"} checked</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left text-sm">
              <thead className="bg-paper text-xs font-semibold uppercase tracking-[0.08em] text-muted">
                <tr>
                ...
                {result.rows.map((row) => (
                  <tr key={row.rowNumber}>
                    ...
                    <td className="px-4 py-3">
                      <p className="font-semibold text-ink">{row.raterName}</p>
                      <p className="text-xs text-muted">{row.raterEmail}</p>
                    </td>
                    <td className="px-4 py-3">{row.relationshipType}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-lg px-2 py-1 text-xs font-semibold ${row.status === "valid" ? "bg-green-soft text-green" : "bg-red-soft text-red"}`}>
                        {row.status}
                      </span>
                    </td>
