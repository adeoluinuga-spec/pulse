...
export default function StrategyForm({data,node,parentId,busy,onSave,onDirty}:{data:StrategyData;node:StrategyRow|null;parentId:string;busy:boolean;onSave:(body:Record<string,unknown>)=>Promise<void>;onDirty:()=>void}) {
 ...
 return <form onChangeCapture={onDirty} onSubmit={e=>{e.preventDefault();const validation=validateNode(draft,{parentKind:data.nodes.find(n=>n.id===draft.parentId)?.kind??null});if(!validation.ok){setErrors(validation.errors);return;}setErrors([]);void onSave({...validation.node,status:draft.status,id:node?.id});}}>
  ...
  <div className={s.grid2}><label className={s.field}>Level<select aria-label="Level" value={draft.kind} onChange={e=>setDraft({...draft,kind:e.target.value as typeof draft.kind})}>{NODE_KINDS.map(kind=><option key={kind} value={kind}>{KIND_LABEL[kind]}</option>)}</select></label>
  <label className={s.field}>Parent level<select aria-label="Parent level" value={draft.parentId} onChange={e=>setDraft({...draft,parentId:e.target.value})}><option value="">Top level</option>{data.nodes.filter(n=>!descendants.has(n.id)).map(n=><option value={n.id} key={n.id}>{n.title}</option>)}</select></label>
  ...
  <label className={s.field}>Accountable owner<select aria-label="Accountable owner" value={draft.ownerId} onChange={e=>setDraft({...draft,ownerId:e.target.value})}><option value="">Not assigned</option>{data.people.map(person=><option key={person.id} value={person.id}>{person.name??person.email}</option>)}</select></label>
  ...
  <label className={s.field}>Status<select aria-label="Status" value={draft.status} onChange={e=>setDraft({...draft,status:e.target.value})}>{["draft","on_track","at_risk","behind","completed"].map(value=><option key={value} value={value}>{value.replaceAll("_"," ")}</option>)}</select></label>
  ...
  <details className={s.notice} open={Boolean(node && (node.measure || node.target_value !== null))}><summary>Direct measure (optional)</summary><p className={s.muted}>Children take precedence in the roll-up. A direct measure is used when this level has no child levels or attached evidence.</p>
   ...
   <div className={s.grid2}><label className={s.field}>Measure type<select aria-label="Measure type" value={draft.measureType} onChange={e=>setDraft({...draft,measureType:e.target.value as typeof draft.measureType})}>{MEASURE_TYPES.map(value=><option key={value} value={value}>{value}</option>)}</select></label>
   <label className={s.field}>Direction<select aria-label="Direction" value={draft.measureDirection} onChange={e=>setDraft({...draft,measureDirection:e.target.value as typeof draft.measureDirection})}><option value="higher">Higher is better</option><option value="lower">Lower is better</option></select></label>
  ...
  {draft.strategies.map((a,i)=><div className={s.notice} key={i}><label className={s.field}>Action {i+1}<input required value={a.statement} onChange={e=>changeStrategy(i,"statement",e.target.value)}/></label>
   ...
   <div className={s.grid2}><label className={s.field}>Responsible person<select aria-label="Responsible person" value={a.responsibleId??""} onChange={e=>changeStrategy(i,"responsibleId",e.target.value)}><option value="">Not assigned</option>{data.people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select></label>
