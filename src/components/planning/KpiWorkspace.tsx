 ...
 return <main className={s.workspace}><PlanningNav/>
  ...
  <Dialog open={form!==null} title={editing?"Edit KPI":"Create a KPI"} onClose={close} busy={busy} error={error}>
   {form&&<form onSubmit={e=>{e.preventDefault();void save();}} onChangeCapture={()=>setDirty(true)}><fieldset disabled={busy}>
    ...
    <div className={s.grid2}><label className={s.field}>Owner<select aria-label="Owner" required value={form.employeeId} onChange={e=>setForm({...form,employeeId:e.target.value})}>{owners.map(person=><option key={person.id} value={person.id}>{person.name??person.email}</option>)}</select></label><label className={s.field}>Strategy level<select aria-label="Strategy level" value={form.strategyNodeId} disabled={Boolean(strategyError)} onChange={e=>setForm({...form,strategyNodeId:e.target.value})}><option value="">Standalone</option>{nodes.map(n=><option key={n.id} value={n.id}>{n.title}</option>)}</select></label></div>
